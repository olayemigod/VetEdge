...
def _display_value(value: Any, fieldtype: str | None = None) -> Any:
	"""Format exported date values without changing report/filter transport values."""
	if value in (None, ""):
		return ""
	type_name = str(fieldtype or "").lower()
	try:
...
def _display_filter_value(key: str, value: Any) -> Any:
	name = str(key or "").lower()
	if name.endswith("_datetime") or name in {"generated_on", "creation", "modified"}:
		return _display_value(value, "Datetime")
	if name.endswith("_date") or name in {"date", "from_date", "to_date"}:
		return _display_value(value, "Date")
	return _safe_value(value)
...
def _filter_rows(filters: dict) -> list[list[Any]]:
	return [[str(key).replace("_", " ").title(), _display_filter_value(key, value)] for key, value in filters.items() if value not in (None, "", [])]
...
def _chart_payload(chart: dict | None) -> tuple[str, list, list[dict]]:
	...
	labels = [
		_display_value(label, "Datetime" if isinstance(label, str) and (" " in label or "T" in label) else "Date")
		if isinstance(label, str) and len(label) >= 10 and label[4:5] == "-" and label[7:8] == "-"
		else label
		for label in list(data.get("labels") or [])[:MAX_PRESENTATION_CHART_POINTS]
	]
	datasets = []
...
def _table_matrix(
	...
	if options["include_generated_metadata"]:
		matrix.append([_("Generated"), _display_value(frappe.utils.now_datetime(), "Datetime")])
	...
	for row in rows:
		matrix.append([_display_value(row.get(column["fieldname"]), column.get("fieldtype")) for column in columns])
...
def download_report_export(
	...
	if file_format == "pdf":
		...
		matrix, _header_index = _table_matrix(report_name, filters_dict, columns, rows, summary, export_options, chart=chart)
