...
def assert_consultation_write_ownership(
	...
	*,
	...
	if doc is None:
		return
	if doc.get("consulting_practitioner") == user:
		return
	verb = {"save": _("save"), "transition": _("update"), "vitals": _("record vitals for")}.get(action, _("update"))
	frappe.throw(_("Doctors can only {0} consultations assigned to themselves.").format(verb), frappe.PermissionError)
...
def enforce_consultation_practitioner_ownership(doc, method: str | None = None) -> None:
	...
	if previous and previous.get("consulting_practitioner") != user:
		frappe.throw(_("This consultation belongs to another doctor and cannot be reassigned or saved by you."), frappe.PermissionError)
	selected = doc.get("consulting_practitioner")
	if selected and selected != user:
		frappe.throw(_("A doctor cannot save a consultation for another doctor."), frappe.PermissionError)
...
def get_clinical_context_options(kind: str, search: str = "", limit: int = 20) -> list[dict[str, Any]]:
	...
	or_filters = None
	if query:
		or_filters = [["Consultation Type", "name", "like", f"%{query}%"], ["Consultation Type", "consultation_type", "like", f"%{query}%"]]
	order_by = "sort_order asc, consultation_type asc" if meta.has_field("sort_order") else "consultation_type asc"
	rows = frappe.get_list("Consultation Type", fields=fields, filters=filters, or_filters=or_filters, order_by=order_by, page_length=page_len)
	return [{"value": row.get("name"), "label": row.get("consultation_type") or row.get("name"), "description": row.get("description") or ""} for row in rows]
...
def get_patient_owner_context(patient: str) -> dict[str, Any]:
	...
	patient_row = frappe.db.get_value("Veterinary Patient", patient, patient_fields, as_dict=True)
	if not patient_row:
		frappe.throw(_("Veterinary Patient could not be found."), frappe.DoesNotExistError)
	owner_name = patient_row.get("primary_owner")
	owner: dict[str, Any] = {"name": owner_name, "label": _document_title("Customer", owner_name) if owner_name else None, "mobile_no": None, "email_id": None}
	if owner_name:
		...
		owner.update({"label": customer_row.get("customer_name") or owner.get("label") or owner_name, "mobile_no": customer_row.get("mobile_no"), "email_id": customer_row.get("email_id")})
	return {"patient": {"name": patient_row.get("name"), "label": patient_row.get("patient_name") or patient_row.get("name"), "default_branch": patient_row.get("default_branch"), "species": patient_row.get("species"), "breed": patient_row.get("breed"), "emergency_contact": patient_row.get("emergency_contact")}, "owner": owner}
