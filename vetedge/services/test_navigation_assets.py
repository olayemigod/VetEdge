...
class TestNavigationAssets(TestCase):
	def test_app_launcher_identity_and_route(self):
		...
		self.assertEqual(assignments["add_to_apps_screen"][0]["title"], "Veterinary")
	...
	def test_desktop_icon_fixture_uses_supported_launcher_route(self):
		...
		self.assertEqual(icon["label"], "Veterinary")
	...
	def test_dashboard_shell_uses_semantic_card_types_and_shared_preset_guard(self):
		...
		dates = (APP_ROOT / "public" / "js" / "edgesuite_date_ranges.js").read_text()
		self.assertIn("card.value_type || card.fieldtype || card.format", shell)
		self.assertIn("DateRanges.applyPreset", shell)
		self.assertIn("state.is_updating_preset", dates)
		self.assertIn('return { start: "", end: "" }', dates)
