...
def _require_export_permissions() -> None:
    ...
    for doctype, label in (
        ...
        if not frappe.has_permission(doctype, "read"):
            frappe.throw(_("You do not have permission to read {0} required for NADIS export.").format(label), frappe.PermissionError)
...
def _patient_species_map(patient_names: list[str]) -> dict[str, str]:
    ...
    for chunk in _chunked(names):
        for row in frappe.get_list(PATIENT_DOCTYPE, filters={"name": ["in", chunk]}, fields=["name", "species"], page_length=len(chunk)):
...
def _branch_mapping(branch_names: list[str]) -> dict[str, dict]:
    ...
    rows = frappe.get_list(BRANCH_DOCTYPE, filters={"name": ["in", names]}, fields=fields, page_length=len(names))
    return {row.get("name"): row for row in rows}


def _month_name(value) -> str:
    return get_datetime(value).strftime("%B").upper()
...
def _validation_result(source_rows: list[dict]) -> dict:
    ...
    for row in source_rows:
        ...
        source_species = patients.get(row.get("patient"))
        ...
        if vaccine_type and vaccine_type not in VACCINATION_VACCINE_TYPES:
            warnings.append({"record": record, "message": _("Vaccine Type is not present in the supplied workbook dropdown; confirm the current NADIS terminology before submission.")})
...
def _aggregate(prepared: list[dict]) -> list[dict]:
    counts = Counter(
        (
            row["country"],
            row["admin_level_1"],
...
def _official_rows(filters: str | dict | None = None) -> dict:
    ...
    aggregated = _aggregate(validation["prepared"])
...
def download_nadis_vaccination_workbook(filters: str | dict | None = None):
    """Generate the official mapped vaccination workbook or fail before download.
