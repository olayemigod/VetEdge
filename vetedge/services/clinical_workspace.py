...
def get_consultations(
	...
	if practitioner:
		filters["consulting_practitioner"] = practitioner
	if patient:
	...
	if query:
		or_filters = [
			["Veterinary Consultation", fieldname, "like", f"%{query}%"]
			for fieldname in ("name", "consultation_title", "patient", "primary_owner", "presenting_complaint")
		]
	...
	return {
		"rows": rows,
		"total": _permission_count(filters, or_filters),
		"start": start,
		"page_length": page_length,
	}
	...
	return {
		...
		"values": {
			**{field: doc.get(field) for field in EDITABLE_SCALAR_FIELDS},
			...
			"dispensary_confirmed_on": doc.dispensary_confirmed_on,
			"dispensary_confirmed_by": doc.dispensary_confirmed_by,
			"dispensary_stock_entry": doc.dispensary_stock_entry,
			...
			"planned_treatments": _serialize_rows(
				doc.get("planned_treatments"),
				PLANNED_TREATMENT_EDITABLE_FIELDS + PLANNED_TREATMENT_IMMUTABLE_FIELDS + ("amount",),
			),
...
def _replace_simple_table(doc, fieldname: str, rows: list[dict], fields: tuple[str, ...]) -> None:
	doc.set(fieldname, [])
	for raw in rows or []:
		row = {field: raw.get(field) for field in fields if raw.get(field) not in (None, "")}
...
def perform_consultation_action(name: str, action: str, modified: str | None = None) -> dict:
	...
	if not action.startswith("status:"):
		frappe.throw(_("Unsupported consultation action."), frappe.ValidationError)
	target = action.split(":", 1)[1]
	transition_consultation_status(name, target)
	return get_consultation_detail(name)
...
def create_consultation_vitals(name: str, values: str | dict | None = None, modified: str | None = None) -> dict:
	...
	_assert_timestamp(doc.doctype, doc.name, modified)
	require_vetedge_platform_access(
		action="clinical_workspace_create_vitals",
		reference_doctype=doc.doctype,
		reference_name=doc.name,
	)
	vitals_name = create_vitals_from_consultation(name, values)
	return {"vitals": vitals_name, "detail": get_consultation_detail(name)}
...
def get_clinical_link_options(kind: str, search: str = "", branch: str | None = None, limit: int = 20) -> list[dict]:
	...
	if kind == "treatment_item":
		rows = get_treatment_item_link_options("Item", query, "name", 0, page_len, {})
		return [{"value": row[0], "label": row[1]} for row in rows]
	...
	if kind not in config:
		frappe.throw(_("Unsupported clinical link type."), frappe.ValidationError)
	doctype, label_field, filters = config[kind]
	meta = frappe.get_meta(doctype)
	...
	if kind == "branch":
		...
		if not user_has_global_branch_access(user):
			filters["name"] = ["in", get_assigned_branches(user)]
	fields = ["name"]
	...
	rows = frappe.get_list(
		...
		filters=filters,
		or_filters=or_filters,
		order_by=f"{label_field if label_field in fields else 'name'} asc",
