...
def classify_service_line(
	...
	description_text = cstr(description or "").lower()

	# Historical consultation invoices may pre-date the current configured item.
	# The source description generated by Consultation Billing is therefore a
	# stronger signal than the broad Treatment Item master.
	if any(token in description_text for token in ("consultation fee", "consultation service", "consult fee")):
		return "Consultation Service"

	if item and configured.get(item):
		return configured[item]

	haystack = f"{item} {description_text}".lower()
...
def _consultation_practitioners(invoice_rows: list[dict]) -> dict[str, dict[str, str]]:
	consultations = {
	...
	name_field = next(
		(
			fieldname
			for fieldname in ("consulting_practitioner_name", "practitioner_name")
	...
	user_field = next(
		(
			fieldname
			for fieldname in ("consulting_practitioner", "practitioner", "doctor", "veterinarian")
			if meta.get_field(fieldname)
		),
		None,
	)
	if not name_field and not user_field:
		return {}

	fields = ["name"]
	for fieldname in (name_field, user_field):
	...
	user_ids = {
		cstr(row.get(user_field)).strip()
		for row in rows
		if user_field and cstr(row.get(user_field)).strip()
	}
	user_labels = {}
	if user_ids and _existing_doctype("User"):
		user_labels = {
			cstr(row.get("name")): cstr(row.get("full_name") or row.get("name")).strip()
	...
	for row in rows:
		user = cstr(row.get(user_field) if user_field else "").strip()
		label = cstr(row.get(name_field) if name_field else "").strip()
		if not label:
			label = user_labels.get(user) or user
		result[cstr(row.get("name"))] = {"label": label, "user": user}
	return result
...
def build_service_revenue_rows(filters=None, dataset: list[dict] | None = None) -> list[dict]:
	...
	for invoice_name, invoice in invoice_map.items():
		practitioner_context = practitioners_by_consultation.get(
			cstr(invoice.get("consultation_reference")),
			{},
		)
		rows.extend(
			_allocate_invoice(
				...
				configured,
				cstr(practitioner_context.get("label")),
				cstr(practitioner_context.get("user")),
			)
		)
	...
	if practitioner_filter:
		rows = [
			...
			if practitioner_filter
			in {
				cstr(row.get("practitioner") or "").strip(),
				cstr(row.get("practitioner_user") or "").strip(),
			}
		]
	return rows
