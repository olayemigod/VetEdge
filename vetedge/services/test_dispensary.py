...
class TestDispensaryFlow(TestCase):
	...
	def test_cancelled_consultation_preserves_stock_reference_without_reposting(self):
		doc = make_consultation()
		doc.status = "Cancelled"
		doc.dispensary_status = DISPENSARY_CONFIRMED
		doc.dispensary_stock_entry = "STE-0002"
		doc.set(
			"dispensed_treatments",
			[
				frappe._dict(
					...
					dispensed_qty=2,
					stock_posted=1,
					stock_entry_reference="STE-0002",
					selected_batch="BATCH-002",
				)
			],
		)
		...
		self.assertEqual(doc.dispensary_stock_entry, "STE-0002")
		self.assertEqual(doc.dispensed_treatments[0].stock_entry_reference, "STE-0002")
		self.assertEqual(doc.dispensed_treatments[0].stock_posted, 1)
		create_stock.assert_not_called()
