...
class TestConsultationBillingPlan(TestCase):
	def test_lab_order_adds_one_consultation_plan_row_per_test(self):
		...
		order = frappe._dict(
			...
			status="Requested",
			lab_tests=[
				frappe._dict(name="ROW-1", lab_test_template="CBC", lab_test_name="Complete Blood Count", billing_item="LAB-CBC", notes="Urgent"),
	...
	def test_lab_order_resave_does_not_duplicate_existing_source_rows(self):
		consultation = make_consultation(
			planned_treatments=[
				frappe._dict(
		...
		order = frappe._dict(
			...
			consultation="VCON-001",
			status="Requested",
			lab_tests=[frappe._dict(name="ROW-1", lab_test_template="CBC", lab_test_name="Complete Blood Count", billing_item="LAB-CBC")],
		)
