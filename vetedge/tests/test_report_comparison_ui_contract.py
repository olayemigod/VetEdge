...
def test_comparison_request_is_aggregate_only_and_does_not_reload_detail_provider():
    source = SOURCE.read_text(encoding="utf-8")
    method = source.split("async loadComparison()", 1)[1].split("async loadGrouping(dimension)", 1)[0]
...
def test_filter_refresh_invalidates_old_comparison_but_pagination_does_not_refetch_it():
    ...
    assert "this.invalidateInsightRequests()" in set_filter
    assert "this.comparison = null" in set_filter
    assert "if (resetPage)" in refresh
    assert "this.invalidateInsightRequests()" in refresh
    assert "this.comparison = null" in refresh
    assert "loadComparison" not in pagination


def test_comparison_ignores_late_response_after_filter_signature_changes():
    source = SOURCE.read_text(encoding="utf-8")
    method = source.split("async loadComparison()", 1)[1].split("async loadGrouping(dimension)", 1)[0]

    assert "const generation = ++this.comparisonRequestGeneration" in method
    assert "const signature = this.insightRequestSignature()" in method
    assert "generation !== this.comparisonRequestGeneration" in method
    assert "signature !== this.insightRequestSignature()" in method
    assert "this.comparison = response.message || null" in method
    assert method.index("generation !== this.comparisonRequestGeneration") < method.index("this.comparison = response.message || null")
