...
GROOMING_BRIDGE = ROOT / "vetedge" / "services" / "appointment_grooming_bridge.py"
VACCINATION_BRIDGE = ROOT / "vetedge" / "services" / "appointment_vaccination_bridge.py"
...
COMPONENT = ROOT / "vetedge" / "public" / "js" / "vetedge_resource_center" / "VetEdgeAppointmentFlow.vue"
RESOURCE_COMPONENT = ROOT / "vetedge" / "public" / "js" / "vetedge_resource_center" / "VetEdgeResourceCenter.vue"
...
SERVICE_COMPONENT = ROOT / "vetedge" / "public" / "js" / "vetedge_service_operations" / "VetEdgeServiceOperations.vue"
APPOINTMENT_CONTROLLER = ROOT / "vetedge" / "veterinary" / "doctype" / "veterinary_appointment" / "veterinary_appointment.py"
APPOINTMENT_CLIENT = ROOT / "vetedge" / "veterinary" / "doctype" / "veterinary_appointment" / "veterinary_appointment.js"
APPOINTMENT_SCHEMA = ROOT / "vetedge" / "veterinary" / "doctype" / "veterinary_appointment" / "veterinary_appointment.json"
SESSION_CONTROLLER = ROOT / "vetedge" / "veterinary" / "doctype" / "pet_grooming_session" / "pet_grooming_session.py"
SESSION_SCHEMA = ROOT / "vetedge" / "veterinary" / "doctype" / "pet_grooming_session" / "pet_grooming_session.json"
CONSULTATION_CONTROLLER = ROOT / "vetedge" / "veterinary" / "doctype" / "veterinary_consultation" / "veterinary_consultation.py"
VACCINATION_CONTROLLER = ROOT / "vetedge" / "veterinary" / "doctype" / "veterinary_vaccination_record" / "veterinary_vaccination_record.py"
VACCINATION_SCHEMA = ROOT / "vetedge" / "veterinary" / "doctype" / "veterinary_vaccination_record" / "veterinary_vaccination_record.json"
LOADER = ROOT / "vetedge" / "veterinary" / "page" / "vetedge_resource_center" / "vetedge_resource_center.js"
...
def test_new_appointment_creation_keeps_boarding_out_of_the_scheduler():
	content = read(API)
...
def test_smart_appointment_context_is_server_enforced_and_backward_compatible():
	...
	assert 'doc.get("appointment_type") == "Vaccination"' in controller
	assert "validate_vaccination_veterinary_appointment(doc)" in controller
...
def test_resource_center_uses_typed_deep_links_and_contextual_clinical_create():
	bundle = read(BUNDLE)
	...
	for contract in (
		...
		"vaccinationAwareAppointmentFlow",
...
def test_grooming_appointment_bridge_preserves_existing_workflow():
	bridge = read(GROOMING_BRIDGE)
...
def test_vaccination_appointment_bridge_is_idempotent_and_financially_safe():
	bridge = read(VACCINATION_BRIDGE)
	schema = read(VACCINATION_SCHEMA)
	controller = read(VACCINATION_CONTROLLER)
	appointment_controller = read(APPOINTMENT_CONTROLLER)
	...
	for contract in (
		...
		"validate_vaccination_veterinary_appointment",
		...
		"create_or_open_vaccination_record_from_appointment",
	...
	assert "Sales Invoice" not in bridge
	assert "Stock Entry" not in bridge
	assert ".submit(" not in bridge
	assert '"fieldname": "linked_appointment"' in schema
	assert '"unique": 1' in schema
	assert '"read_only": 1' in schema
...
def test_smart_appointment_actions_create_or_open_linked_service_records():
	...
	for contract in (
		...
		'"create_vaccination_record", "Create Vaccination Record"',
		'"open_vaccination_record",',
		'"Open Vaccination Record"',
		"_vaccination_record_route",
		"create_or_open_vaccination_record_from_appointment",
	...
	assert "vaccination_workflow" not in actions
	assert "Open Vaccination Workflow" not in actions
