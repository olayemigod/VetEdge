...
frappe.pages['vetedge-master-workspace'].on_page_show = function(wrapper) {
	...
	if (wrapper.vue_app) {
		try {
			...
			console.warn('Error unmounting Veterinary Masters Vue app:', error);
		}
	...
	const $loading = $('<div class="p-6 text-center text-muted"></div>')
		.text(__('Loading Veterinary masters...'))
		.appendTo(page.body);

	const showFailure = (message) => {
		$loading.remove();
		$('<div class="alert alert-danger p-6 text-center"></div>')
			.text(message || __('Veterinary masters failed to load.'))
			.appendTo(page.body);
	};
	...
	frappe.require('edgesuite_ui.bundle.js', () => {
		...
		if (!runtime?.createEdgeApp || missing.length || !supportsDocuments) {
			showFailure(
				!supportsDocuments
					? __('Veterinary master pages require EdgeSuite UI 0.5.0 or newer. Loaded version: {0}', [runtime?.version || __('unknown')])
					: missing.length
						? __('Missing EdgeSuite UI master components: {0}', [missing.join(', ')])
		...
		const mountWorkspace = () => {
			...
			frappe.require('vetedge_master_workspace.bundle.js', () => {
				if (wrapper.current_visit_id !== visitId) return;
				if (!window.VetEdgeMasterWorkspace) {
					showFailure(__('The Veterinary Masters product bundle is unavailable.'));
					return;
				}
				try {
					$loading.remove();
					const root = $('<div class="vetedge-master-workspace-root" data-edge-product="vetedge"></div>')
						.appendTo(page.body);
					wrapper.vue_app = runtime.createEdgeApp(window.VetEdgeMasterWorkspace);
					wrapper.vue_app.mount(root[0]);
				} catch (error) {
					console.error('Error mounting Veterinary Masters:', error);
					showFailure(__('Error mounting Veterinary Masters: {0}', [error.message || String(error)]));
				}
			});
		};
