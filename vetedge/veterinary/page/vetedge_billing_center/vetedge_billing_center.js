frappe.pages['vetedge-billing-center'].on_page_load = function(wrapper) {
	wrapper.page = frappe.ui.make_app_page({ parent: wrapper, title: __('Billing Center'), single_column: true });
};

frappe.pages['vetedge-billing-center'].on_page_show = function(wrapper) {
	...
	$(page.body).empty();
	const loading = $('<div class="p-6 text-center text-muted"></div>').text(__('Loading Billing Center...')).appendTo(page.body);
	const showFailure = (message) => {
		loading.remove();
		$('<div class="alert alert-danger p-6 text-center"></div>').text(message || __('Billing Center failed to load.')).appendTo(page.body);
	};
	...
	frappe.require('edgeui.bundle.js', () => {
		...
		if (!runtime?.createEdgeApp || missing.length) {
			showFailure(missing.length
				? __('Billing Center requires EdgeSuite UI 0.6.3 or newer. Missing: {0}', [missing.join(', ')])
		...
		const mountWorkspace = () => {
			...
			frappe.require('vetedge_billing_center.bundle.js', () => {
				...
				try {
					loading.remove();
					const root = $('<div class="vetedge-billing-center-root" data-edge-product="vetedge"></div>').appendTo(page.body);
					wrapper.vue_app = window.mountVetEdgeBillingCenter(root[0]);
				} catch (error) {
					console.error('Error mounting Billing Center:', error);
					showFailure(__('Error mounting Billing Center: {0}', [error.message || String(error)]));
				}
			});
		};
