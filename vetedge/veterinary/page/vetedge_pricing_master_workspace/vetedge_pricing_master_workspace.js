...
frappe.pages['vetedge-pricing-master-workspace'].on_page_show = function(wrapper) {
	...
	wrapper.vue_app?.unmount?.(); wrapper.vue_app = null;
	$(page.body).empty();
	...
	frappe.require('edgeui.bundle.js', () => {
		...
		const mountWorkspace = () => {
			...
			frappe.require('vetedge_pricing_master_workspace.bundle.js', () => {
				...
				try { $loading.remove(); const root = $('<div class="vetedge-pricing-master-workspace-root" data-edge-product="vetedge"></div>').appendTo(page.body); wrapper.vue_app = window.mountVetEdgePricingMasterWorkspace(root[0]); }
