	...
	frappe.require('edgesuite_ui.bundle.js', () => {
		...
		const mountWorkspace = () => {
			...
			frappe.require('vetedge_clinical_workspace.bundle.js', () => {
				...
				frappe.require([
					'/assets/vetedge/js/vetedge_medical_history_ui.js',
					'/assets/vetedge/js/vetedge_clinical_workspace_phase5.js'
				])
					.then(() => {
						if (wrapper.current_visit_id !== visitId) return;
						try {
							if (!window.VetEdgeMedicalHistoryUI || !window.VetEdgeClinicalWorkspacePhase5) {
								throw new Error('Required Phase 5 clinical modules were not registered.');
							}
					...
					.catch((error) => {
						console.error('Phase 5 clinical assets could not load:', error);
						showFailure(__('Veterinary Clinical Workspace assets could not load: {0}', [error?.message || String(error)]));
					});
			});
		};
