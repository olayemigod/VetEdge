	...
	>
				...
				<div v-else class="clinical-detail">
					...
					<section v-if="activeTab === 'visit'" class="clinical-panel">
						...
						<div class="clinical-grid">
							...
							<div class="clinical-datetime-field">
								<label for="vetedge-consultation-datetime">Consultation Date/Time</label>
								<div class="clinical-datetime-picker">
									<span :class="['clinical-datetime-value', { 'is-placeholder': !form.consultation_datetime }]">{{ displayEditableDateTime(form.consultation_datetime) || 'DD-MM-YYYY HH:mm' }}</span>
									<span class="clinical-datetime-hint" aria-hidden="true">▾</span>
									<input id="vetedge-consultation-datetime" class="clinical-datetime-native" :value="form.consultation_datetime" type="datetime-local" aria-label="Consultation Date/Time" @change="(event) => updateDateTimeField('consultation_datetime', event)" />
								</div>
					...
					<section v-if="activeTab === 'treatment'" class="clinical-panel">
						...
						<div class="clinical-datetime-field">
							<label for="vetedge-follow-up-datetime">Follow-up Date/Time</label>
							<div class="clinical-datetime-picker">
								<span :class="['clinical-datetime-value', { 'is-placeholder': !form.follow_up_date }]">{{ displayEditableDateTime(form.follow_up_date) || 'DD-MM-YYYY HH:mm' }}</span>
								<span class="clinical-datetime-hint" aria-hidden="true">▾</span>
								<input id="vetedge-follow-up-datetime" class="clinical-datetime-native" :value="form.follow_up_date" type="datetime-local" aria-label="Follow-up Date/Time" @change="(event) => updateDateTimeField('follow_up_date', event)" />
							</div>
							<small>DD-MM-YYYY HH:mm</small>
						</div>
					</section>
		...
		<EdgeModal :open="clinicalMasterDialog.open" :title="clinicalMasterTitle" subtitle="Create the missing reusable clinical master without leaving this workflow." :busy="clinicalMasterDialog.saving" @close="closeClinicalMaster">
			...
			<div v-else-if="clinicalMasterDialog.kind === 'treatment_item'" class="clinical-grid">
				...
				<EdgeInput :model-value="clinicalMasterDialog.values.default_price" type="number" min="0" step="0.01" label="Default Price" :disabled="clinicalMasterDialog.pricingLocked" :description="clinicalMasterDialog.pricingLocked ? 'Existing Item Price is locked for your role.' : 'Uses the selected/contextual Price List.'" @update:model-value="(value) => setClinicalMasterValue('default_price', value)" />
...
export default {
	...
	methods: {
		...
		displayEditableDateTime(value) {
			const serverValue = serverDatetime(value);
			return window.VetEdgeDateTime?.formatDateTime?.(serverValue, "") || serverValue || "";
		},
		updateDateTimeField(field, event) { this.updateField(field, event?.target?.value || ""); },
		...
		async selectTreatmentBaseItem(value) {
			...
			try {
				const itemContext = await call(API.itemCreationContext, {
					...
					company: this.form.company || undefined,
					customer: this.form.primary_owner || undefined,
					price_list: this.clinicalMasterDialog.values.price_list || undefined,
				});
				if (this.clinicalMasterDialog.values.item !== value) return;
				if (itemContext?.treatment_item_exists) {
					this.clinicalMasterDialog.values = { ...this.clinicalMasterDialog.values, item: "", item_label: "" };
					frappe.show_alert({ message: __("That ERPNext Item already has a Veterinary Treatment Item. Return to the treatment search and select it."), indicator: "orange" });
					return;
				}
				this.clinicalMasterDialog.pricingLocked = Boolean(itemContext?.price_locked);
				this.clinicalMasterDialog.values = {
					...this.clinicalMasterDialog.values,
					item_label: itemContext?.item_name || value,
					price_list: itemContext?.price_list || this.clinicalMasterDialog.values.price_list || "",
					default_price: itemContext?.existing_item_price ? Number(itemContext.existing_price_rate || 0) : 0,
				};
			} catch (error) {
				frappe.show_alert({ message: message(error, __("Item pricing context could not be loaded.")), indicator: "orange" });
			}
		},
