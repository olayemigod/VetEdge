	...
	>
			...
			<div v-else-if="episode.name" class="hospitalisation-episode">
				<section class="episode-statusbar">
					<div class="episode-statuses">
						...
						<span v-if="episode.signals?.pending_stock">Stock Pending: <strong>{{ episode.signals.pending_stock }}</strong></span>
						...
						<article v-for="row in episode.activities" :key="row.name" class="episode-list-row">
							<div class="episode-list-main">
								<div class="episode-list-heading">
									...
									<EdgeStatusBadge v-if="row.stock_affecting" :label="row.stock_status || 'Pending'" :status="row.stock_status || 'Pending'" />
								</div>
				...
				<section v-if="activeTab === 'care'" class="episode-panel">
					<header class="episode-panel-header">
						<div><h3>Care Location & Stock</h3><p>Care location and stock posting continue to use the authoritative Hospitalisation services.</p></div>
					</header>
					...
					<div class="episode-two-column">
						...
						<div class="episode-card">
							...
							<div class="episode-actions">
								<button type="button" class="edge-button" :disabled="busy || dirty" @click="previewStock">Preview Stock</button>
				...
				<section v-if="activeTab === 'billing'" class="episode-panel">
					...
					<div v-else class="episode-charge-table">
						<div class="episode-charge-head"><span>Charge</span><span>Qty</span><span>Rate</span><span>Amount</span><span>Status</span><span>Invoice</span></div>
				...
				<section v-if="activeTab === 'discharge'" class="episode-panel">
					...
					<div v-if="dischargeReadiness" :class="['episode-guidance', dischargeReadiness.can_discharge ? 'is-success' : 'is-warning']">
						...
						<div class="episode-readiness-grid">
							...
							<span>Stock activities: <strong>{{ (dischargeReadiness.pending_stock_activities || []).length }}</strong></span>
		...
		<EdgeModal :open="activityDialog.open" :title="`Add ${activityDialog.type || 'Activity'}`" subtitle="Hospitalisation Clinical Care" :busy="busy" @close="closeActivity">
			<div class="episode-grid">
				...
				<EdgeLinkField :model-value="activityDialog.item" :selected-label="activityDialog.item_label" label="Item" placeholder="Optional unless billable or stock affecting" :searcher="(query) => optionSearch('item', query)" @update:model-value="selectActivityItem" />
				...
				<EdgeDropdown :model-value="String(activityDialog.stock_affecting || 0)" label="Stock Affecting" :options="yesNoOptions" @update:model-value="(value) => activityDialog.stock_affecting = Number(value || 0)" />
			...
			<template #footer><button type="button" class="edge-button" :disabled="busy" @click="closeActivity">Cancel</button><button type="button" class="edge-button edge-button--primary" :disabled="busy" @click="saveActivity">Add Activity</button></template>
		</EdgeModal>
		...
		<EdgeModal :open="vaccinationDialog.open" title="Add Vaccination" subtitle="Hospitalisation Clinical Care" :busy="busy" @close="closeVaccination">
			<div class="episode-grid">
				...
				<EdgeDropdown :model-value="String(vaccinationDialog.values.stock_affecting)" label="Stock Affecting" :options="yesNoOptions" @update:model-value="(value) => setVaccination('stock_affecting', Number(value || 0))" />
			...
			<template #footer><button type="button" class="edge-button" :disabled="busy" @click="closeVaccination">Cancel</button><button type="button" class="edge-button edge-button--primary" :disabled="busy || !vaccinationDialog.values.vaccine" @click="saveVaccination">Add Vaccination</button></template>
		</EdgeModal>
		...
		<EdgeModal :open="labDialog.open" title="Add Lab Order" subtitle="Hospitalisation Clinical Care" :busy="busy" @close="closeLab">
			<div class="episode-stack">
				<p v-if="!episode.linked_consultation" class="episode-warning">This is a direct admission. Selected tests will be recorded as Hospitalisation Lab activities because there is no linked Consultation to create a separate Lab Order.</p>
			...
			<template #footer><button type="button" class="edge-button" :disabled="busy" @click="closeLab">Cancel</button><button type="button" class="edge-button edge-button--primary" :disabled="busy || !labDialog.selected.length" @click="saveLab">Add Lab Tests</button></template>
		</EdgeModal>
...
export default {
	...
	computed: {
		...
		stockActivities() { return (this.episode.activities || []).filter((row) => row.stock_affecting); },
		stockPreviewRows() {
	...
	methods: {
		...
		async selectActivityItem(value) {
			...
			try {
				...
				if (context?.is_stock_item) this.activityDialog.stock_affecting = 1;
		...
		async saveActivity() {
			if (!this.activityDialog.type || this.busy) return;
			this.busy = true; this.error = '';
			try {
				const result = await call(API.addActivity, {
					...
					stock_affecting: this.activityDialog.stock_affecting,
		...
		openVaccination() { this.vaccinationDialog = { ...blankVaccination(), open: true, values: { ...blankVaccination().values, administered_on: currentLocalDatetime() } }; },
		...
		async saveVaccination() {
			this.busy = true; this.error = '';
			try {
				const values = { ...this.vaccinationDialog.values, administered_on: serverDatetime(this.vaccinationDialog.values.administered_on) };
		...
		async discharge() {
			...
			if (result.blocked) {
				...
				if (result.open_stock_action) this.previewStock(true);
				return;
			}
...
.episode-card{display:grid;gap:.65rem}.episode-card>span{color:var(--edge-color-ink-500,#617589);font-size:.75rem}.episode-card>strong{font-size:1.2rem}.episode-card>small{color:var(--edge-color-ink-500,#617589)}
...
.episode-charge-table{border:1px solid var(--edge-color-border,#dfe6ec);border-radius:.75rem;overflow:hidden}.episode-charge-head,.episode-charge-row{align-items:center;display:grid;gap:.75rem;grid-template-columns:minmax(12rem,2fr) .7fr 1fr 1fr 1fr 1.1fr;padding:.75rem}.episode-charge-head{background:var(--edge-color-surface-muted,#f5f8fc);font-size:.75rem;font-weight:700}.episode-charge-row{border-top:1px solid var(--edge-color-border,#dfe6ec)}.episode-charge-row>div{display:grid;gap:.2rem}.episode-charge-row small{color:var(--edge-color-ink-500,#617589)}
...
@media(max-width:1100px){.episode-tabs{grid-template-columns:repeat(3,minmax(0,1fr))}.episode-action-grid,.episode-summary-grid,.episode-readiness-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.episode-charge-head,.episode-charge-row{grid-template-columns:minmax(10rem,2fr) .6fr 1fr 1fr 1fr}.episode-charge-head>span:last-child,.episode-charge-row>:last-child{display:none}}
@media(max-width:720px){.episode-statusbar,.episode-panel-header,.episode-list-row{align-items:stretch;flex-direction:column}.episode-tabs,.episode-grid,.episode-two-column,.episode-action-grid,.episode-summary-grid,.episode-readiness-grid{grid-template-columns:minmax(0,1fr)}.episode-wide{grid-column:auto}.episode-list-meta{align-items:start;text-align:left}.episode-charge-table{overflow-x:auto}.episode-charge-head,.episode-charge-row{min-width:44rem}.episode-inline-picker{grid-template-columns:minmax(0,1fr)}}
</style>
