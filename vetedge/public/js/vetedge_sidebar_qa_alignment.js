(function () {
	...
	function redirectCurrentLegacyRoute() {
		...
		const care = careLocationTargetFromNativePath(path);
		if (care) {
			const target = new URL(care, window.location.origin);
			for (const [key, value] of new URLSearchParams(window.location.search || "")) {
				if (!target.searchParams.has(key)) target.searchParams.set(key, value);
			}
			sameTab(`${target.pathname}${target.search}`, { replace: true });
			return true;
		}
		return false;
	}
	...
	function reportNameFromCandidate(candidate) {
		const explicit = String(candidate?.dataset?.linkTo || candidate?.getAttribute?.("data-link-to") || "").trim();
	...
	function isCareLocationCandidate(candidate) {
		const explicit = String(candidate?.dataset?.linkTo || candidate?.getAttribute?.("data-link-to") || "").trim();
		if (explicit === CARE_LOCATION_DOCTYPE || explicit === "vetedge-care-locations") return true;
		const text = itemLabel(candidate);
