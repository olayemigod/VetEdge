...
function wrapGuardedMethod(methods, methodName) {
	...
	const wrapped = function (...args) {
		if (!this.dirty || typeof this.confirmDiscard !== "function") return original.apply(this, args);
...
export function applyWorkspaceSafety(component, { guardNavigation = false } = {}) {
	...
	methods.confirmDiscard = methods.confirmDiscard || function (action) {
		...
		this.openConfirmation({
			...
			message: typeof __ === "function" ? __("You have unsaved changes. Continue without saving them?") : "You have unsaved changes. Continue without saving them?",
			confirmLabel: typeof __ === "function" ? __("Discard Changes") : "Discard Changes",
			danger: true,
			handler: async () => { this.dirty = false; return action(); },
		});
	};
	...
	methods.confirmPendingAction = async function () {
		...
		try {
			...
			window.frappe?.msgprint?.({ title: typeof __ === "function" ? __("Action failed") : "Action failed", message: error?.message || error?._server_messages || String(error), indicator: "red" });
		}
	};
	if (guardNavigation) {
		for (const methodName of ["backToList", "openRoute", "reloadCurrentView"]) wrapGuardedMethod(methods, methodName);
	}
	component.mounted = wrapHook(component.mounted, function () { window.addEventListener("beforeunload", this.handleBeforeUnload); });
	component.beforeUnmount = wrapHook(component.beforeUnmount, function () { window.removeEventListener("beforeunload", this.handleBeforeUnload); });
	return component;
}
