...
- Phase 9D.1 consultation cancellation resolution decisions: paid/partly paid consultation cancellation now has a structured decision layer before any future financial workflow proceeds. Added `Veterinary Consultation Cancellation Resolution` to record the consultation, patient, customer, branch, company, selected resolution action, status, reason, selected by/on, optional reschedule links, billing-group paid/outstanding snapshot, and linked invoice snapshot. The service API validates cancellation preflight first, only allows recordable financial actions returned by preflight, limits recording to System Manager, VetEdge Administrator, Branch Manager, Accounts/Cashier, Accounts User, and equivalent VetEdge/accounting roles, prevents overwriting approved/completed decisions, and never cancels the consultation or creates refunds, credit notes, Payment Entries, Stock Entries, or Sales Invoice reversals. The Cancel Consultation dialog now shows an existing decision and records a selected resolution action with a required note. Remaining risk: Phase 9D.1 records intent only; Phase 9D.2 should define approval/completion rules and any explicit admin/accounting workflows for retain-payment clinical cancellation, refunds, credits, or reschedules.
