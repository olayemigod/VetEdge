...
- Phase 9A consultation cancellation safety preflight: cancelling a paid/partly paid consultation is a clinical and financial resolution decision, not a simple status change. Implemented behavior: `consultation_cancellation` evaluates Billing Group Invoice History, lab orders, vaccination records, hospitalisation records, Stock Entries, Billing Sessions, and notification references using explicit VetEdge links only. Direct cancellation is blocked when submitted invoices, paid amounts, active hospitalisation, final lab/vaccination records, or submitted stock entries exist; draft invoices and early draft clinical dependencies are returned as warnings. The Cancel Consultation UI now calls the preflight before status transition, and backend status transitions validate through the same helper. Submitted Sales Invoices, Payment Entries, Stock Entries, and consultation invoice history are not mutated. Tests cover safe cancellation, draft warnings, submitted unpaid invoices, paid/partly paid multiple-invoice groups, clinical/stock blockers, and transition-path enforcement. Remaining risk: Phase 9A does not automate refunds, credit notes, submitted stock reversal, or draft dependency cleanup; Phase 9B should define clinic policy workflows for those resolution actions.
- Billing Group vs Patient Outstanding Context: current service billing group truth must come only from explicit source evidence such as current consultation invoice references, Billing Session Charges, direct source fields, source markers, or explicitly related service documents. Older invoices for the same patient/customer are informational/action-only and live in a separate patient outstanding context; they must not satisfy the current consultation payment gate or block current consultation cancellation unless explicitly linked into the current billing group. The Billing Modal now exposes those rows separately as “Other Outstanding Invoices for this Patient” with clear copy that payment does not count toward the current consultation unless linked. Remaining risk: patient outstanding context uses customer/patient evidence for display convenience only and must not be reused by workflow gates.
